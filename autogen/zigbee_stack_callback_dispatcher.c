...
emberAfPluginNetworkSteeringMarker();
}
...
emAfPluginReportingStackStatusCallback(status);
}
...
emAfPluginGreenPowerClientDGpSentCallback(status, gpepHandle);
}
...
emAfPluginGreenPowerClientGpepIncomingMessageCallback(status, gpdLink, sequenceNumber, addr, gpdfSecurityLevel, gpdfSecurityKeyType, autoCommissioning, bidirectionalInfo, gpdSecurityFrameCounter, gpdCommandId, mic, proxyTableIndex, gpdCommandPayloadLength, gpdCommandPayload);
}
